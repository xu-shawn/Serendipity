...
public class AlphaBeta
{
	...
	private void updateContinuationHistories(int ply, int depth, Board board, Move move, List<Move> quietsSearched)
	{
		...
		for (int i : new int[] { 1, 2, 3, 4, 6 })
		{
			...
			if (i == 3)
			{
				conthist.register(board, move, bonus / 8);

				for (Move quietMove : quietsSearched)
				{
					conthist.register(board, quietMove, malus / 8);
				}
			}

			else if (i == 6)
			{
				conthist.register(board, move, bonus / 2);

				for (Move quietMove : quietsSearched)
				{
					conthist.register(board, quietMove, malus / 2);
				}
			}
